...
# IMPROVEMENT: Compile regex once at module level (not per UDF call)
...
def compute(ctx, mds, model, md_output):
    ...
    def analyse_page(text):
        ...
        try:
            ...
            # IMPROVEMENT: Use pre-compiled pattern
            # Find all base64 images and collect them
            matches = IMAGE_PATTERN.findall(text)

            if not matches:
                return text  # No images, return as-is
            ...
            for explanation, image_type, base64_data in matches:
                if base64_data not in image_dict:
                    # IMPROVEMENT: Validate image size to prevent OOM
            ...
            # Get analysis for each unique base64 image
            for img_b64 in image_dict.keys():
                if image_dict[img_b64] is None:  # Only process if not already marked as too large
                    analysis = get_chart_analysis_fromBase64(img_b64)
                    image_dict[img_b64] = analysis

            # IMPROVEMENT: Single pass replacement using lambda
    ...
    # CRITICAL FIX: Add has_image column detection
